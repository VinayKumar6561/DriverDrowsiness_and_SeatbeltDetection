...
drowsy_frames = 0
drowsy_threshold = 5# You can adjust this threshold
...
while True:
    ...
    for left_eye, right_eye in zip(left_eyes, right_eyes):
        ...
        drowsy_message_time=None
        # Display the result on the frame
        if prediction[0] == 1:  # Assuming 1 represents drowsy in your model
            while drowsy_message_time:
                drowsy_message_time-=1
            ...
            # Increment drowsy_frames counter
             ...
             if drowsy_frames >= drowsy_threshold:
                cv2.putText(frame, " Drowsy", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2) # Display the message for seconds
                
                if drowsy_message_time is None:
                     drowsy_message_time = time.time()  # Record the time when the message is displayed
                elif time.time() - drowsy_message_time >=2000:
                        drowsy_frames = 0  # Reset drowsy_frames counter
                        drowsy_message_time = None  # Reset the timer
